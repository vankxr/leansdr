...
void output_initial_info(FILE* f, const config& cfg)
{
    ...
    if(cfg.standard == config::DVB_S)
    {
        fprintf(f, "CONSTELLATION %s%s%s\n", quote,
            cstln_base::names[cfg.constellation], quote);
...
struct runtime_common
{
    ...
    runtime_common(const config& cfg)
    {
        ...
        switch(cfg.input_format)
        {
            ...
            {
                pipebuf<cu8>* p_stdin = new pipebuf<cu8>(sch, "stdin", BUF_BASEBAND + cfg.input_buffer);
                file_reader<cu8>* r_stdin = new file_reader<cu8>(sch, 0, *p_stdin);
                r_stdin->loop = cfg.loop_input;
                cconverter<u8, 128, f32, 0, 1, 1>* r_convert = new cconverter<u8, 128, f32, 0, 1, 1>(sch, *p_stdin, *p_rawiq);
                amp = 128;
            }
            break;
            case config::INPUT_S8:
            {
                pipebuf<cs8>* p_stdin = new pipebuf<cs8>(sch, "stdin", BUF_BASEBAND + cfg.input_buffer);
                file_reader<cs8>* r_stdin = new file_reader<cs8>(sch, 0, *p_stdin);
                r_stdin->loop = cfg.loop_input;
                cconverter<s8, 0, f32, 0, 1, 1>* r_convert = new cconverter<s8, 0, f32, 0, 1, 1>(sch, *p_stdin, *p_rawiq);
                amp = 128;
            }
            break;
            ...
            {
                pipebuf<cs16>* p_stdin = new pipebuf<cs16>(sch, "stdin", BUF_BASEBAND + cfg.input_buffer);
                file_reader<cs16>* r_stdin = new file_reader<cs16>(sch, 0, *p_stdin);
                r_stdin->loop = cfg.loop_input;
                cconverter<s16, 0, f32, 0, 1, 1>* r_convert = new cconverter<s16, 0, f32, 0, 1, 1>(sch, *p_stdin, *p_rawiq);
                amp = (cfg.input_format == config::INPUT_S12 ? 2048 : 32768);
            }
            break;
            case config::INPUT_F32:
            {
                pipebuf<cf32>* p_stdin = new pipebuf<cf32>(sch, "stdin", BUF_BASEBAND + cfg.input_buffer);
                file_reader<cf32>* r_stdin = new file_reader<cf32>(sch, 0, *p_stdin);
                r_stdin->loop = cfg.loop_input;
                scaler<float, cf32, cf32>* r_scale = new scaler<float, cf32, cf32>(sch, cfg.float_scale, *p_stdin, *p_rawiq);
                amp = 2.0;
            }
            break;
            default:
        ...
        if(cfg.awgn)
        {
            ...
            pipebuf<cf32>* p_noise = new pipebuf<cf32>(sch, "noise", BUF_BASEBAND);
            wgn_c<f32>* r_noise = new wgn_c<f32>(sch, *p_noise);
            r_noise->stddev = cfg.awgn;
            pipebuf<cf32>* p_noisy = new pipebuf<cf32>(sch, "noisy", BUF_BASEBAND);
            adder<cf32>* r_addnoise = new adder<cf32>(sch, *p_preprocessed, *p_noise, *p_noisy);
        ...
        if(cfg.anf)
        {
            pipebuf<cf32>* p_autonotched = new pipebuf<cf32>(sch, "autonotched", BUF_BASEBAND);
            auto_notch<f32>* r_auto_notch = new auto_notch<f32>(sch, *p_preprocessed, *p_autonotched, cfg.anf, 0);
        ...
        if(cfg.Fderot)
        {
            ...
            pipebuf<cf32>* p_derot = new pipebuf<cf32>(sch, "derotated", BUF_BASEBAND);
            rotator<f32>* r_derot = new rotator<f32>(sch, *p_preprocessed, *p_derot, -cfg.Fderot / cfg.Fs);
        ...
        if(cfg.fd_spectrum)
        {
            ...
            spectrum<f32, 1024>* r_spectrum = new spectrum<f32, 1024>(sch, *p_preprocessed, *p_spectrum);
        ...
        if(cfg.resample)
        {
            ...
            pipebuf<cf32>* p_resampled = new pipebuf<cf32>(sch, "resampled", BUF_BASEBAND);
            float* coeffs;
            ...
            if(cfg.debug)
                filtergen::dump_filter("lowpass", ncoeffs, coeffs);

            r_resample = new fir_filter<cf32, float>(sch, ncoeffs, coeffs, *p_preprocessed, *p_resampled, decimpp);
        ...
        if(!cfg.resample && cfg.decim > 1)
        {
            ...
            pipebuf<cf32>* p_decimated = new pipebuf<cf32>(sch, "decimated", BUF_BASEBAND);
            decimator<cf32>* p_decim = new decimator<cf32>(sch, decimpp, *p_preprocessed, *p_decimated);
        ...
        if(cfg.gui)
        {
            cscope<f32>* r_cscope_pp = new cscope<f32>(sch, *p_preprocessed, -amp, amp, "preprocessed (iq)");
            spectrumscope<f32>* r_fft_pp = new spectrumscope<f32>(sch, *p_preprocessed, amp, "preprocessed (spectrum)");
        ...
        if(cfg.fd_pp >= 0)
        {
            ...
            file_writer<cf32>* r_ppout = new file_writer<cf32>(sch, *p_preprocessed, cfg.fd_pp);
        }
        ...
        switch(cfg.sampler)
        {
            ...
            case config::SAMP_RRC:
            {
                ...
                int ncoeffs = filtergen::root_raised_cosine(order, cfg.Fm / Frrc, cfg.rolloff, &coeffs);
                ...
                sampler = new fir_sampler<float, float>(ncoeffs, coeffs, rrc_steps);
            }
            break;
            default:
        ...
        p_vber = new pipebuf<float>(sch, "VBER", BUF_SLOW);
        rate_estimator<float>* r_vber = new rate_estimator<float>(sch, *p_verrcount, *p_vbitcount, *p_vber);
        // About twice per second, and slow enough for 2e-5 resolution
        // TBD r_vber->sample_size = cfg.Fm/2;  // About twice per second, depending
        // on CR
        ...
        if(cfg.fd_info >= 0)
        {
            file_printer<f32>* r_printfreq = new file_printer<f32>(sch, "FREQ %.0f\n", *p_freq, cfg.fd_info);
            ...
            new file_printer<unsigned long>(sch, "LOCKTIME %lu\n", *p_locktime, cfg.fd_info, decimation(cfg.Fm / 8 / 204, cfg.Finfo)); // TBD CR
        ...
        slowmultiscope<f32>::chanspec chans[] = {
            ...
            },
            {
                ...
                -10,
                20,
                (r_cnr ? slowmultiscope<f32>::chanspec::ASYNC : slowmultiscope<f32>::chanspec::DISABLED)
            },
            {
                p_tscount,
                "TS recovery",
                "%3.0f %%",
                {255, 255, 0},
                110 / max_packets_per_pixel,
                0,
                101,
                (slowmultiscope<f32>::chanspec::flag)(slowmultiscope<f32>::chanspec::ASYNC | slowmultiscope<f32>::chanspec::SUM)
            },
        };

        if(cfg.gui)
        {
            slowmultiscope<f32>* r_scope_timeline = new slowmultiscope<f32>(sch, chans, sizeof(chans) / sizeof(chans[0]), "timeline");
...
int run_dvbs2(config& cfg)
{
    ...
    s2_frame_receiver<f32, llr_ss> demod(run.sch, run.sampler, *run.p_preprocessed, p_slots, run.p_freq, run.p_ss, run.p_mer, run.p_cstln, run.p_cstln_pls, run.p_iqsymbols, run.p_framelock);
    ...
    if(cfg.fd_const)
    {
        ...
        if(cfg.json)
            symbol_printer = new file_carrayprinter<f32>(run.sch, "SYMBOLS [", "[%.0f,%.0f]", ",", "]\n", *run.p_cstln, cfg.fd_const);
        else
            symbol_printer = new file_carrayprinter<f32>(run.sch, "SYMBOLS %d", " %.0f,%.0f", "", "\n", *run.p_cstln, cfg.fd_const);
    ...
    if(!cfg.ldpc_helper)
    {
        ...
        pipebuf<fecframe<hard_sb>>* p_fecframes = new pipebuf<fecframe<hard_sb>>(run.sch, "FEC frames", BUF_FRAMES);
        new s2_deinterleaver<llr_ss, hard_sb>(run.sch, p_slots, *p_fecframes);
        // Decode FEC-protected frames into plain BB frames.
        s2_fecdec<bool, hard_sb>* r_fecdec = new s2_fecdec<bool, hard_sb>(run.sch, *p_fecframes, p_bbframes, run.p_vbitcount, run.p_verrcount);
    ...
    else
    {
        ...
        // TBD Latency
        pipebuf<fecframe<llr_sb>>* p_fecframes = new pipebuf<fecframe<llr_sb>>(run.sch, "FEC frames", BUF_FRAMES);
        new s2_deinterleaver<llr_ss, llr_sb>(run.sch, p_slots, *p_fecframes);
        // Decode FEC-protected frames into plain BB frames.
        s2_fecdec_helper<llr_t, llr_sb>* r_fecdec = new s2_fecdec_helper<llr_t, llr_sb>(run.sch, *p_fecframes, p_bbframes, cfg.ldpc_helper, run.p_vbitcount, run.p_verrcount);
...
int run_dvbs(config& cfg)
{
    ...
    cstln_receiver<f32, softsymb> demod(run.sch, run.sampler, *run.p_preprocessed, p_symbols, run.p_freq, run.p_ss, run.p_mer, run.p_cstln);

    if(cfg.constellation != cstln_base::QPSK && cfg.constellation != cstln_base::BPSK)
    ...
    {
        if(cfg.verbose)
            fprintf(stderr, "PLL parameters for low SNR\n");
    ...
    if(cfg.viterbi)
    {
        if(cfg.fec == FEC23 && (demod.cstln->nsymbols == 4 || demod.cstln->nsymbols == 64))
        {
            if(cfg.verbose)
                fprintf(stderr, "Handling rate 2/3 as 4/6\n");
            cfg.fec = FEC46;
        }

        viterbi_sync* r = new viterbi_sync(run.sch, p_symbols, p_bytes, demod.cstln, cfg.fec);
    ...
    if(cfg.hdlc)
    {
        pipebuf<u8>* p_descrambled = new pipebuf<u8>(run.sch, "descrambled", BUF_MPEGBYTES);
        new etr192_descrambler(run.sch, p_bytes, *p_descrambled);
        pipebuf<u8>* p_frames = new pipebuf<u8>(run.sch, "frames", BUF_MPEGBYTES);
    ...
    if(!cfg.hdlc)
    {
        mpeg_sync<u8, 0>* r_sync = new mpeg_sync<u8, 0>(run.sch, p_bytes, p_mpegbytes, r_deconv, run.p_lock, run.p_locktime);
    ...
    rs_decoder<u8, 0> r_rsdec(run.sch, p_rspackets, p_rtspackets, run.p_vbitcount, run.p_verrcount);
        ...
        if(c)
        {
            ...
            if(cfg.json)
            {
                ...
                for(int i = 0; i < c->nsymbols; ++i)
                    fprintf(f, "%s[%d,%d]", i ? "," : "", c->symbols[i].re, c->symbols[i].im);
    ...
    if(cfg.fd_spectrum >= 0)
    {
        ...
        slowmultiscope<f32>::chanspec chans[] = {
            {&p_freq, "estimated frequency", "Offset %3.3f kHz", {0, 255, 255}, cfg.Fs * 1e-3f, (cfg.Ftune - cfg.Fm / 2) * 1e-3f, (cfg.Ftune + cfg.Fm / 2) * 1e-3f, slowmultiscope<f32>::chanspec::WRAP},
            {&p_ss, "signal strength", "Signal Strength %3.3f", {255, 0, 0}, 1, 0, 128, slowmultiscope<f32>::chanspec::DEFAULT},
            {&p_mer, "MER", "MER %5.1f dB", {255, 0, 255}, 1, -10, 20, slowmultiscope<f32>::chanspec::DEFAULT},
            {&p_cnr, "CNR", "CNR %5.1f dB", {255, 255, 0}, 1, -10, 20, (r_cnr ? slowmultiscope<f32>::chanspec::ASYNC : slowmultiscope<f32>::chanspec::DISABLED)},
            {&p_tscount, "TS recovery", "%3.0f %%", {255, 255, 0}, 110 / max_packets_per_pixel, 0, 101, (slowmultiscope<f32>::chanspec::flag) (slowmultiscope<f32>::chanspec::ASYNC | slowmultiscope<f32>::chanspec::SUM)},
        };

        if(cfg.gui)
        {
            slowmultiscope<f32>* r_scope_timeline = new slowmultiscope<f32>(run.sch, chans, sizeof(chans) / sizeof(chans[0]), "timeline");
...
int run_highspeed_s2(config& cfg)
{
    ...
    float amp = 2048;
    if(cfg.gui)
        cscope<s16>* r_cscope_raw = new cscope<s16>(&sch, p_rawiq, -amp, amp, "rawiq (iq)");
#endif
    ...
    if(cfg.gui)
    {
        cscope<float>* r_scope_symbols = new cscope<float>(&sch, p_sampled, -amp, amp);
...
int run_highspeed(config& cfg)
{
    ...
    if(cfg.gui)
        cscope<u8>* r_cscope_raw = new cscope<u8>(&sch, p_rawiq, 0, 2 * amp, "rawiq (iq)");
#endif
...
#define ALGEBRAIC_COMPAT 0 // Use legacy constellation receiver ?
    ...
    cstln_receiver<f32> demod(&sch, p_rawiqf, p_symbols, &p_freq, NULL, NULL, &p_sampledf);
    cstln_lut<256> qpsk(cstln_base::QPSK);
    demod.cstln = &qpsk;
    // Convert the sampled symbols to cu8 for GUI
    cconverter<f32, 0, u8, 128, 1, 1> r_convert_samp(&sch, p_sampledf, p_sampled);
#else
    // Use the new fixed-point receiver
    pipebuf<u8> p_symbols(&sch, "PSK hard symbols", BUF_SYMBOLS);
    fast_qpsk_receiver<u8> demod(&sch, p_rawiq, p_symbols, &p_freq, &p_sampled);
#endif
    demod.set_omega(cfg.Fs / cfg.Fm);
    ...
    if(cfg.gui)
    {
        cscope<u8>* r_scope_symbols = new cscope<u8>(&sch, p_sampled, 0, 2 * amp);
        r_scope_symbols->decimation = 1;
    }
#endif
    ...
    mpeg_sync<u8, 0> r_sync(&sch, p_bytes, p_mpegbytes, NULL, &p_lock, &p_locktime);
    ...
    rs_decoder<u8, 0> r_rsdec(&sch, p_rspackets, p_rtspackets, &p_vbitcount, &p_verrcount);
    ...
    if(cfg.fd_const >= 0)
    {
        ...
        if(cfg.json)
            symbol_printer = new file_carrayprinter<u8>(&sch, "SYMBOLS [", "[%.0f,%.0f]", ",", "]\n", p_sampled, cfg.fd_const);
        else
            symbol_printer = new file_carrayprinter<u8>(&sch, "SYMBOLS %d", " %d,%d", "", "\n", p_sampled, cfg.fd_const);
    ...
    slowmultiscope<f32>::chanspec chans[] = {
        {&p_freq, "estimated frequency", "%3.3f kHz", {0, 255, 255}, cfg.Fs * 1e-3f, (cfg.Ftune - cfg.Fm / 2) * 1e-3f, (cfg.Ftune + cfg.Fm / 2) * 1e-3f, slowmultiscope<f32>::chanspec::WRAP},
        {&p_tscount, "TS recovery", "%3.0f %%", {255, 255, 0}, 110 / max_packets_per_pixel, 0, 101, (slowmultiscope<f32>::chanspec::flag)(slowmultiscope<f32>::chanspec::ASYNC | slowmultiscope<f32>::chanspec::SUM)},
    };
    ...
    for(int i = 1; i < argc; ++i)
    {
        ...
        else if(!strcmp(argv[i], "--sampler") && i + 1 < argc)
        {
            ...
            if(!strcmp(argv[i], "nearest"))
                cfg.sampler = config::SAMP_NEAREST;
            else if(!strcmp(argv[i], "linear"))
                cfg.sampler = config::SAMP_LINEAR;
