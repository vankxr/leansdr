...
namespace leansdr
{
    ...
    const struct modcod_info
    {
        ...
        static const int MIN_SYMBOLS_PER_FRAME = (1 + MIN_SLOTS_PER_FRAME) * PLSLOT_LENGTH;
        static const int MAX_SLOTS_PER_FRAME = 360;
        static const int MAX_SYMBOLS_PER_FRAME = (1 + MAX_SLOTS_PER_FRAME) * PLSLOT_LENGTH + ((MAX_SLOTS_PER_FRAME - 1) / 16) * PILOT_LENGTH;
    ...
    struct s2_frame_transmitter : runnable
    {
        ...
        void run()
        {
            while(in.readable() >= 1)
            {
                ...
                int nsymbols = ((1 + nslots) * pin->LENGTH + (pls->pilots ? ((nslots - 1) / 16) * pilot.LENGTH : 0));
    ...
    private:
        ...
        complex<T>* get_csymbols(int modcod)
        {
            if(!pcsymbols[modcod])
            {
                ...
                cstln_lut<hard_ss, 256> cstln(mcinfo->c, mcinfo->esn0_nf, mcinfo->g1, mcinfo->g2, mcinfo->g3);
    ...
    struct s2_frame_receiver : runnable
    {
        ...
        s2_frame_receiver(scheduler* sch, sampler_interface<T>* _sampler, pipebuf< complex<T> >& _in, pipebuf< plslot<SOFTSYMB> >& _out, pipebuf<float>* _freq_out = NULL, pipebuf<float>* _ss_out = NULL, pipebuf<float>* _mer_out = NULL, pipebuf< complex<float> >* _cstln_out = NULL, pipebuf< complex<float> >* _cstln_pls_out = NULL, pipebuf< complex<float> >* _symbols_out = NULL, pipebuf<int>* _state_out = NULL) :
            ...
            cstln(NULL),
            in(_in),
            out(_out, 1 + modcod_info::MAX_SLOTS_PER_FRAME),
            meas_count(0),
        ...
        struct sampler_state
        {
            ...
            char* format()
            {
                ...
                sprintf(buf, "%9.2lf %+6.0f ppm  %+3.0f °  %f", (double)((p - (complex<T>*)NULL) & 262143) + mu, fw16 * 1e6 / 65536, fmodfs(ph16, 65536.0f) * 360 / 65536, gain);
        ...
        void run()
        {
            ...
            // TBD margin ?
            int min_samples = (1 + modcod_info::MAX_SYMBOLS_PER_FRAME + sof.LENGTH + plscodes.LENGTH) * omega0 * 2;
            while(in.readable() >= min_samples + sampler->readahead() && out.writable() >= 1 + modcod_info::MAX_SLOTS_PER_FRAME && opt_writable(freq_out, 1) && opt_writable(ss_out, 1) && opt_writable(mer_out, 1) && opt_writable(symbols_out, modcod_info::MAX_SYMBOLS_PER_FRAME) && opt_writable(state_out, 1))
            {
                if(first_run)
                {
                    enter_frame_detect();
        ...
        void run_frame_probe_locked()
        {
            ...
            for(int s = 0; s < PLH_LENGTH; ++s)
            {
                ...
                typename cstln_lut<SOFTSYMB, 256>::result* cr = (s & 1) ? bpsk->lookup(p.im, -p.re) : bpsk->lookup(p.re, p.im);
            ...
            int llr_sof = 0;
            {
                ...
                for(int i = 0; i < sof.LENGTH; ++i)
                {
                    ...
                    if((sof.VALUE << i) & highbit)
                        llr_sof -= l;  // Expected 1
                    else
                        llr_sof += l;  // Expected 0
                }
            }
            ...
            // Perform ML decoding of PLHEADER to confirm LLR-based result.
            {
                ...
                fprintf(stderr, "CARRIER MLPLH %d %d %4.1f dB %s\n", plscode_index, plscode_index2, 10 * log10f(best_m2), (plscode_index == plscode_index2) ? "OK" : "NOK");
            }
#endif
            ...
            if(sch->debug2)
                fprintf(stderr, "PLS: mc=%2d, sf=%d, pilots=%d llr=%3d %4.1f dB ", pls.modcod, pls.sf, pls.pilots, llr_sof + llr_plscode, mer_plh_db);
            ...
            else
            {
                ...
                if(mer_plh_db < mcinfo->esn0_nf - 1.0f)
                {
                    ...
                    if(sch->debug)
                        fprintf(stderr, "Insufficient MER\n");
            ...
            fprintf(stderr, "\nLOOKAHEAD %d PLH sr %+3.0f  %s  %.1f dB\n", plh_counter, (1 - ss.omega / omega0) * 1e6, ss.format(), 10 * log10f(mer2_plh));
#endif
            // Next SOF
            ...
            {
                ssnext = ss;
                int ns = (S * PLSLOT_LENGTH + (pls.pilots ? ((S - 1) / 16) * pilot.LENGTH : 0));
            ...
            if(pls.pilots)
            {
                ...
                for(int i = 0; i < npilots; ++i)
                {
                    ...
                    mer2_pilots += mer2;
#if DEBUG_LOOKAHEAD
                    fprintf(stderr, "LOOKAHEAD %d PILOT%02d  sr %+3.0f  %s  %.1f dB\n", plh_counter, i, (1 - ssp.omega / omega0) * 1e6, ssp.format(), 10 * log10f(mer2));
#endif
                }
            }
...
#if DEBUG_LOOKAHEAD
            fprintf(stderr, "LOOKAHEAD %d NEXTSOF  sr %+3.0f  %s  %.1f dB\n", plh_counter, (1 - ssnext.omega / omega0) * 1e6, ssnext.format(), 10 * log10f(mer2_pilots / npilots));
            ++plh_counter;
#endif
            ...
            for(int slot = 0; slot < S; ++slot, ++pout)
            {
                ...
                else if((pls.pilots && !(slot & 15) && slot + 16 >= S) || (!pls.pilots && !slot))
                {
            ...
            if(sch->debug2)
                fprintf(stderr, "sr%+.0f fs=%.0f %4.1f dB\n", (1 - ss.omega / omega0) * 1e6, (freq_stats.max() - freq_stats.min()) * 1e6 / 65536.0f, pls.pilots ? 10 * log10f(mer2_pilots / npilots) : -99);
        ...
        void match_freq(complex<float>* expect, complex<float>* recv, int ns, sampler_state* ss)
        {
        ...
        // Idempotent.
        float match_ph_amp(complex<float>* expect, complex<float>* recv, int ns, sampler_state* ss)
        {
            complex<float> rr = 0;
        ...
        void match_frame(sampler_state* pss, const s2_pls* pls, int S, cstln_lut<SOFTSYMB, 256>* dcstln)
        {
        ...
        {
            if(sch->verbose)
                fprintf(stderr, "PL errors: %d/%d (%.0f ppm)\n", pls_total_errors, pls_total_count, 1e6 * pls_total_errors / pls_total_count);
        }
        ...
        complex<float> descramble(sampler_state* ss, const complex<float>& p)
        {
            ...
            {
                case 3:
                    res.re = -p.im; res.im = p.re;
                    break;
                case 2:
                    res.re = -p.re; res.im = -p.im;
                    break;
                case 1:
                    res.re = p.im; res.im = -p.re;
                    break;
        ...
        inline complex<float> interp_next(sampler_state* ss)
        {
            ...
            complex<float> s(ss->p[0].re * cmu + ss->p[1].re * ss->mu, ss->p[0].im * cmu + ss->p[1].im * ss->mu);
            ss->mu += ss->omega;
            // Derotate
        ...
        inline uint8_t track_symbol(sampler_state* ss, const complex<float>& p, cstln_lut<SOFTSYMB, 256>* c)
        {
    ...
    public:
        ...
        cstln_lut<SOFTSYMB, 256>* get_cstln(int modcod)
        {
            if(!cstlns[modcod])
            {
                ...
                cstlns[modcod] = new cstln_lut<SOFTSYMB, 256>(mcinfo->c, mcinfo->esn0_nf, mcinfo->g1, mcinfo->g2, mcinfo->g3);
#if 0
    ...
    private:
        ...
        static void interleave(int bps, int rows, const hard_sb* pin, int nslots, bool msb_first, plslot<hard_ss>* pout)
        {
            void (*func)(int rows, const hard_sb * pin, int nslots, plslot<hard_ss> *pout) = 0;
        ...
        template<int BPS>
        static inline void split_byte(const hard_sb* pi, int stride, hard_sb accs[BPS])
        {
    ...
    private:
        ...
        static void deinterleave(int bps, int rows, const plslot<SOFTSYMB>* pin, int nslots, bool msb_first, SOFTBYTE* pout)
        {
            void (*func)(int rows, const plslot<SOFTSYMB> *pin, int nslots, SOFTBYTE * pout) = 0;
        ...
        static inline void split_symbol(const llr_ss& ps, int bps, hard_sb accs[/*bps*/], int nacc, bool msb_first)
        {
            if(msb_first)
            {
                for(int b = 0; b < bps; ++b)
        ...
        // Fast variant
        template<int MSB_FIRST, int BPS>
        static inline void split_symbol(const llr_ss& ps, hard_sb accs[/*bps*/], int nacc)
        {
            if(MSB_FIRST)
            {
                for(int b = 0; b < BPS; ++b)
        ...
        static inline void split_symbol(const llr_ss& ps, int bps, llr_sb accs[/*bps*/], int nacc, bool msb_first)
        {
            if(msb_first)
            {
                for(int b = 0; b < bps; ++b)
        ...
        // Fast variant
        template<int MSB_FIRST, int BPS>
        static inline void split_symbol(const llr_ss& ps, llr_sb accs[/*bps*/], int nacc)
        {
            if(MSB_FIRST)
            {
        ...
        static inline void pack_qpsk_symbol(const llr_ss& ps, hard_sb* acc, int nacc)
        {
        ...
        static inline void pack_qpsk_symbol(const llr_ss& ps, llr_sb* acc, int nacc)
        {
...
#include "dvbs2_data.h"
    ...
    struct s2_fecdec : runnable
    {
        ...
        void run()
        {
            while(in.readable() >= 1 && out.writable() >= 1 && opt_writable(bitcount, 1) && opt_writable(errcount, 1))
            {
    ...
    struct s2_fecdec_helper : runnable
    {
        ...
        void run()
        {
            ...
            while((all_blocked || !work_done || jobs.full()) && !jobs.empty() && jobs.peek()->h->b_out && out.writable() >= 1 && opt_writable(bitcount, 1) && opt_writable(errcount, 1))
    ...
    private:
        ...
        void spawn_helper(helper_instance* h, const s2_pls* pls)
        {
            ...
            if(fcntl(tx[0], F_SETPIPE_SZ, pipesize) < 0 || fcntl(rx[0], F_SETPIPE_SZ, pipesize) < 0 || fcntl(tx[1], F_SETPIPE_SZ, pipesize) < 0 || fcntl(rx[1], F_SETPIPE_SZ, pipesize) < 0)
            {
                fprintf(stderr,
    ...
    struct s2_framer : runnable
    {
        ...
        void run()
        {
            while(out.writable() >= 1)
            {
                ...
                {
                    if(10 + nremain + tspacket::SIZE * in_ts.readable() < framebytes)
    ...
    private:
        void handle_ts(uint8_t* data, uint16_t dfl, uint16_t syncd, uint8_t sync)
        {
            ...
            {
                // Sanity check
                if(syncd / 8 != tspacket::SIZE - nleftover)
                {
                    if(sch->debug)
            ...
            while(pos + (tspacket::SIZE - nleftover) + 1 <= dfl / 8)
            {
                ...
                memcpy(pout->data + nleftover, data + pos, tspacket::SIZE - nleftover);
                pout->data[0] = sync;  // Replace CRC
                uint8_t crc = crc8.compute(pout->data + 1, tspacket::SIZE - 1);

                if(data[pos + (tspacket::SIZE - nleftover)] == crc)
                {
                    info_good_packet();
                }
                else
                ...
                out_ts.written(1);
                pos += tspacket::SIZE - nleftover;
                nleftover = 0;
            }

            int remain = dfl / 8 - pos;

            if(nleftover + remain > sizeof(leftover))
