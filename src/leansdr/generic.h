...
namespace leansdr
{
    ...
    struct file_reader : runnable
    {
        ...
        void run_tunnel()
        {
            // struct passed to a tunnel-mode file reader should have a field of size ssize_t at the end
            // which will be filled in with the number of bytes read for each packet
            // typically used with tun interface pdu

            while(out.writable())
            {
                ssize_t nr = read(fdin, out.wr(), sizeof(T) - sizeof(ssize_t));
                ...
                if(nr <= 4) // Should read more than 4 bytes from tun, which is the fixed header size
                    fatal("tun short read");

                *(ssize_t *)((uintptr_t)out.wr() + sizeof(T) - sizeof(ssize_t)) = nr;

                out.written(1);
            }
        }

        void set_realtime(T& _filler = NULL)
        {
            if(fdin < 0)
                return;
    ...
    struct file_writer : runnable
    {
        ...
        void run_tunnel()
        {
            // struct passed to a tunnel-mode file writter should have a field of size ssize_t at the end
            // which should contain the number of bytes to write for each packet
            // typically used with tun interface pdu

            while(in.readable())
            {
                ssize_t nw = *(ssize_t *)((uintptr_t)in.rd() + sizeof(T) - sizeof(ssize_t));

                if(nw <= 4) // Should write more than 4 bytes to tun, which is the fixed header size
                    fatal("tun short write");

                nw = write(fdout, in.rd(), nw);
